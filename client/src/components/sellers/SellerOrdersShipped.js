...
const GET_ORDER_ITEMS_URL = "sellers/orders/shipped";
...
const SellerOrdersShipped = () => {
  ...
  useEffect(() => {
    axios.post(GET_ORDER_ITEMS_URL).then((response) => {
                  ...
                  <div className="grid grid-cols-5 text-center m-8 mt-0 shadow-md rounded-md">
                    ...
                    <div className="flex justify-center text-base items-center w-full ">
                      {new Date(item.DeliveredDate).getDate() +
                        "-" +
                        (new Date(item.DeliveredDate).getMonth() + 1) +
                        "-" +
                        new Date(item.DeliveredDate).getFullYear()}
                    </div>
